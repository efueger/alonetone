...
class AttachedValidator < ActiveModel::EachValidator
  ...
  def validate_content_type(record, attribute, value)
    unless accepted_content_types.include?(value.content_type)
      record.errors.add(
  ...
  def validate_byte_size(record, attribute, value)
    ...
    allow_empty = options.dig(:byte_size, :allow_empty) || false
    if byte_size.zero?
      record.errors.add(attribute, :blank, value: byte_size) unless allow_empty
      # Zero byte size is either allowed or not allowed so we can return here.
      return
    end

    greater_than = options.dig(:byte_size, :greater_than)
    ...
    less_than = options.dig(:byte_size, :less_than)
